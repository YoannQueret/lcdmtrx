...
class LcdMatrix( object ):
	""" Classe Python pour gérer le backpack Adafruit USB+Série LCD disponible chez MCHobby 
	
	Voir produit: http://shop.mchobby.be/product.php?id_product=475
	Voir tuto: http://wiki.mchobby.be/index.php?title=LCD-USB-TTL"""
	
	__serial = None # Serial object to send data to LCD
	
	def __init__( self, lcd_serial_file, baud = 9600 ):
		""" Initialise l'objet et initialize la communication série
		
			Args:
				lcd_serial_file (str): le periphérique série à utiliser (ex: /dev/ttyACM0)
				baud (int): débit en bauds
				
			Remarks:
				Vous pouvez trouver la valeur de "lcd_serial_file" sur un Unix en utilisant
				la commande "dmesg" dans un terminal après avoir branché l'afficheur.
				
				Voyez également le tutoriel mentionné dans l'entête du fichier
		"""
		self.__serial = Serial( lcd_serial_file, baud, timeout=1 )
...
class EuropeLcdMatrix( LcdMatrix ):
	...
	def __init__( self, lcd_serial_file, baud = 9600 ):
		...
		"""
		LcdMatrix.__init__(self, lcd_serial_file, baud)
